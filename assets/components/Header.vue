            ...
            <nav class="w-full flex justify-between items-center flex-wrap lg:hidden">
                ...
                <div class="w-full flex flex-col mt-4" v-if="showMobileMenu">
                    ...
                    <div class="whitespace-no-wrap flex flex-col bg-white border border-gray-300 w-auto mt-2" v-if="coursesMenuVisible">
                        <router-link v-for="course in courses" :key="course.id" :to="{ name: 'Course',  params: { id: course.id } }" ><div class="py-2 px-4 mt-2 transition duration-300 uppercase text-center ease-in-out hover:bg-gray-300" @click="showMobileMenu = false, coursesMenuVisible = false">{{ course.name }}</div></router-link>
                    </div>
